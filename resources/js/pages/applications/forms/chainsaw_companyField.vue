...
watch(
    ...
    async (newProvince) => {
        if (newProvince) {
            try {
                const response = await axios.get(`http://192.168.2.106:8000/api/provinces/${newProvince}/cities`);
...
watch(
    ...
    async (newCityMun) => {
        ...
        if (newCityMun) {
            try {
                const response = await axios.get(`http://192.168.2.106:8000/api/barangays`, {
                    params: {
                        reg_code: region,
                        prov_code: province,
                        mun_code: newCityMun,
