...
const saveCompanyApplication = async () => {
    ...
    try {
        const response = await insertFormData('http://192.168.0.117:8000/api/chainsaw/company_apply', {
            ...company_form,
            ...formData,
            encoded_by: userId,
        });
...
const submitChainsawForm = async () => {
    ...
    try {
        for (const chainsaw of chainsaws) {
            ...
            await axios.post('http://192.168.0.117:8000/api/chainsaw/insertChainsawInfo', formData, 
            {
...
const submitORPayment = async () => {
    ...
    try {
        await axios.post('http://192.168.0.117:8000/api/chainsaw/insert_payment', formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
        });
                ...
                <div class="relative">
                    ...
                    <div v-for="(chainsaw, index) in chainsaws" :key="index"
                        ...
                        <div class="mt-5 grid grid-cols-1 gap-6 md:grid-cols-3">
                            <div hidden>
                                <FloatLabel hidden>
                                    <InputText hidden v-model="applicationData.application_no" class="w-full" />
